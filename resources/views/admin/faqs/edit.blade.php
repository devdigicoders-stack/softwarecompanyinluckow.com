<x-admin-layout pageTitle="Edit Page FAQ">
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            <h3 class="fw-bold text-slate-900 mb-1" style="color: #0f172a;">Edit Page FAQ</h3>
            <p class="text-secondary small mb-0">Modify FAQ question, answer, target page, or display order.</p>
        </div>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <select name="page_name" id="pageNameSelect" class="form-select @error('page_name') is-invalid @enderror" required>
                        ...
                        @foreach($pages as $p)
                            <option value="{{ $p }}" {{ old('page_name', $faq->page_name) == $p ? 'selected' : '' }}>Page: {{ ucfirst($p) }}</option>
                        @endforeach
                        <option value="other" {{ old('page_name') == 'other' || !in_array($faq->page_name, $pages) ? 'selected' : '' }}>Other / Custom Page Slug...</option>
                    </select>
                    @error('page_name')<div class="invalid-feedback">{{ $message }}</div>@enderror

                    <!-- Custom Page Input -->
                    <div id="customPageContainer" class="mt-2 {{ old('page_name') == 'other' || !in_array($faq->page_name, $pages) ? '' : 'd-none' }}">
                        <input type="text" name="custom_page_name" id="custom_page_name" class="form-control @error('custom_page_name') is-invalid @enderror" value="{{ old('custom_page_name', $faq->page_name) }}" placeholder="Type custom page slug...">
                ...
                <div class="col-md-6">
                    ...
                    <input type="number" name="order_index" id="order_index" class="form-control @error('order_index') is-invalid @enderror" value="{{ old('order_index', $faq->order_index) }}" min="1" required>
                    <div class="form-text text-muted small">Lower numbers display first in accordion.</div>
                ...
                <div class="col-12 mt-4 pt-3 border-top">
                    <button type="submit" class="btn btn-glass-primary px-4 py-2.5 fw-bold">
                        <i class="bi bi-save me-1"></i> Update FAQ Item
                    </button>
